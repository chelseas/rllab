...
from sandbox.rocky.tf.core.network import MLP, SimpleMLP, MLP2
...
# avoid unecessary tf ops. still has x'W' multiplication
class GaussianMLP2Policy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        with tf.variable_scope(name):
            ...
            LayersPowered.__init__(self, [l_mean, l_std_param])
            super(GaussianMLP2Policy, self).__init__(env_spec)
...
# implemented for W@x + b multiplication unlike the others which are implemented for x'@W' + b' multiplication
class SimpleGaussianMLPPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        with tf.variable_scope(name):
            ...
            if std_network is not None:
                ...
                if adaptive_std:
                    ...
                    if std_parametrization == 'exp':
                        ...
                        raise NotImplementedError
                    l_std_param = L.SimpleParamLayer(
                        mean_network.input_layer,
                        num_units=action_dim,
                        param=tf.constant_initializer(init_std_param),
                        name="output_std_param",
            ...
            LayersPowered.__init__(self, [l_mean, l_std_param])
            super(SimpleGaussianMLPPolicy, self).__init__(env_spec)
    ...
    def get_actions(self, observations):
        flat_obs = self.observation_space.flatten_n_Wx(observations)
        means, log_stds = self._f_dist(flat_obs)
        rnd = np.random.normal(size=means.shape)
        actions = rnd * np.exp(log_stds) + means
        #import pdb; pdb.set_trace()
